...
class Car:
    def __init__(self, parent, world, init_angle, init_x, init_y):
        self.parent = parent
        self.world = world
    ...
    def step(self, dt):
        for w in self.wheels:
            ...
            grass = True
            friction_limit = self.parent.friction_limit*0.6  # Grass friction if no tile
            for tile in w.tiles:
                friction_limit = max(friction_limit, self.parent.friction_limit*tile.road_friction)
                grass = False
            ...
            w.omega += dt*self.parent.engine_power*w.gas/self.parent.wheel_moment_of_inertia/(abs(w.omega)+5.0)  # small coef not to divide by zero
            self.fuel_spent += dt*self.parent.engine_power*w.gas
            ...
            w.omega -= dt*f_force*w.wheel_rad/self.parent.wheel_moment_of_inertia
